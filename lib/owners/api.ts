...
import { createBankAccount, createPhone } from "@/lib/owners/form";
import type {
  BankAccountType,
  MarriageRegime,
  PixType,
...
export interface OwnerWritePayload {
  name: string;
  email: string;
  document: string;
  type: "person" | "enterprise";
...
const MARRIAGE_REGIME_MAP: Record<string, MarriageRegimeApi> = {
  ...
  "Contract pré-nupcial": "prenuptial_agreement",
};
...
const MARRIAGE_REGIME_REVERSE: Record<MarriageRegimeApi, MarriageRegime> = {
  ...
  prenuptial_agreement: "Contract pré-nupcial",
};
...
export function buildPersonPayload(data: OwnerFormData): OwnerWritePayload {
...
export function mapPersonDtoToListRow(p: PersonDto): OwnerListRow {
  const doc = p.document ?? "";
  const digits = onlyDigits(doc);
  const type: OwnerListRow["type"] =
...
export function personDtoToFormData(dto: PersonDto): OwnerFormData {
...
export function apiCreateOwner(
  accessToken: string,
  data: OwnerFormData,
): Promise<void> {
...
export function apiUpdateOwner(
  accessToken: string,
  personId: string,
  data: OwnerFormData,
): Promise<void> {
...
export function apiDeleteOwner(
  accessToken: string,
  personId: string,
): Promise<void> {
  return apiFetch<void>(`/persons/v1/${personId}`, {
