...
export function ContractSelect({
  ...
  const list =
    open && position && mounted ? (
      <ul
        ref={listRef}
        id={listId}
        className="contract-create-custom-select__list contract-create-custom-select__list--portal"
        role="listbox"
        style={{
          position: 'fixed',
          top: position.top,
        ...
        {options.map((opt) => (
          <li key={opt.value || '__empty'} role="presentation">
            <button
              ...
              className={cn(
                'contract-create-custom-select__option',
                value === opt.value && 'contract-create-custom-select__option--selected',
              )}
  ...
  return (
    <div
      ref={rootRef}
      className={cn(
        'contract-create-custom-select',
        open && 'contract-create-custom-select--open',
        className,
      )}
    >
      <button
        ...
        id={id}
        disabled={disabled}
        className="contract-create-custom-select__trigger contract-create-select"
      ...
      >
        <span
          className={cn(
            'contract-create-custom-select__value',
            showPlaceholder && 'contract-create-custom-select__value--placeholder',
          )}
        >
        ...
        <ChevronDown
          size={16}
          className="contract-create-custom-select__chevron"
          aria-hidden
        />
      </button>
