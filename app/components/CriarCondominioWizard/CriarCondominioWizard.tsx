...
import { StepCondominioIdentificacao } from './StepCondominioIdentificacao'
import { StepCondominioEndereco } from './StepCondominioEndereco'

export interface CondominioWizardProps {
  mode?: 'create' | 'edit'
  initialData?: CondominiumFormData
  condominiumId?: string
}

export function CriarCondominioWizard({
  mode = 'create',
  initialData,
  condominiumId,
}: CondominioWizardProps = {}) {
  ...
  const footerConfig = useMemo(() => {
    ...
    if (step === 'endereco') {
      return {
        hint: CONDOMINIUM_STEP_HINTS.endereco,
  ...
  function renderStep() {
    switch (step) {
      case 'identificacao':
        return (
          <StepCondominioIdentificacao data={data} errors={errors} patch={patch} />
        )
      case 'endereco':
        return (
          <StepCondominioEndereco
            data={data}
            errors={errors}
