...
import type { Category, Subcategory, ImovelFilters } from '@/lib/imoveis/api'
import { useCallback, useMemo, useState } from 'react'
import './imoveis-filter.css'
...
function emptyFilters(): ImovelFilters {
  return {}
}
...
interface ImoveisFilterDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  categories: Category[]
  subcategories: Subcategory[]
  currentFilters: ImovelFilters
  onApply: (filters: ImovelFilters) => void
}

export function ImoveisFilterDialog({
  open,
  onOpenChange,
  categories,
  subcategories,
  currentFilters,
  onApply,
}: ImoveisFilterDialogProps) {
  const [draft, setDraft] = useState<ImovelFilters>(() => ({ ...currentFilters }))

  const resetDraft = useCallback(() => {
    setDraft({ ...currentFilters })
  }, [currentFilters])

  function patch(partial: Partial<ImovelFilters>) {
  ...
  return (
    ...
    >
      <DialogContent>
        ...
        <DialogBody>
          <div className="imoveis-filter-grid">
            <SelectField
              label="Status"
              value={draft.status ?? ''}
              onChange={(e) => patch({ status: (e.target.value || undefined) as ImovelFilters['status'] })}
              options={STATUS_OPTIONS}
              placeholder="Todos"
              fieldClassName="imoveis-filter-field"
              labelClassName="imoveis-filter-label"
            />

            <div className="imoveis-filter-field">
              <label className="imoveis-filter-label" htmlFor="filter-code">Código</label>
              <input
                id="filter-code"
                type="text"
            ...
            <div className="imoveis-filter-field">
              <label className="imoveis-filter-label" htmlFor="filter-city">Cidade</label>
              <input
                id="filter-city"
                type="text"
                className="imoveis-filter-input"
            ...
            <div className="imoveis-filter-field">
              <label className="imoveis-filter-label" htmlFor="filter-state">Estado</label>
              <input
                id="filter-state"
                type="text"
                maxLength={2}
          ...
          <div className="imoveis-filter-range">
            <div className="imoveis-filter-range-header">
              <span className="imoveis-filter-label">Valor do aluguel</span>
              <span className="imoveis-filter-range-values">
                {formatBRL(priceMin)} — {priceMax >= PRICE_MAX ? `${formatBRL(PRICE_MAX)}+` : formatBRL(priceMax)}
              </span>
            </div>

            <div className="imoveis-filter-slider-track">
              <div
                className="imoveis-filter-slider-fill"
                style={{ left: `${fillLeft}%`, width: `${fillRight - fillLeft}%` }}
              />
            </div>

            <div className="imoveis-filter-slider-inputs">
              <input
                type="range"
                min={PRICE_MIN}
                max={PRICE_MAX}
        ...
        <DialogFooter>
          <div className="imoveis-filter-actions">
            <button type="button" className="imoveis-filter-btn-clear" onClick={handleClear}>
              Limpar filtros
            </button>
            <button type="button" className="imoveis-filter-btn-apply" onClick={handleApply}>
              Aplicar
            </button>
          </div>
        </DialogFooter>
